...
impl<T: KvsEngine> KvServer<T> {
	...
	pub fn new(addr: impl ToSocketAddrs, store: T) -> Result<Self> {
	...
	fn process_command(&mut self, command: Command) -> Response {
		match command {
			Command::Get { key } => {
				self.process_get(key)
			},
			Command::Set { key, val } => {
				self.process_set(key, val)
			},
			Command::Remove { key } => {
	...
	fn process_get(&mut self, key: String) -> Response {
		...
		match result {
			Ok(val) => Response::success(val.unwrap_or(String::from("Key not found"))),
