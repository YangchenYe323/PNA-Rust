...
// `kvs-client4` with no args should exit with a non-zero code.
#[test]
fn client_cli_no_args() {
    let temp_dir = TempDir::new().unwrap();
    let mut cmd = Command::cargo_bin("kvs-client4").unwrap();
...
// `kvs-client4 -V` should print the version
#[test]
...
// `kvs-server4 -V` should print the version
#[test]
fn server_cli_version() {
    let temp_dir = TempDir::new().unwrap();
    let mut cmd = Command::cargo_bin("kvs-server4").unwrap();
    cmd.args(&["-V"])
        .current_dir(&temp_dir)
        .assert()
        .stdout(contains(env!("CARGO_PKG_VERSION")));
...
fn cli_access_server(engine: &str, addr: &str) {
    ...
    let temp_dir = TempDir::new().unwrap();
    let mut server = Command::cargo_bin("kvs-server4").unwrap();
